...
```{r}
val_loss <- data.frame(
  epoch = seq_along(original_hist$metrics$val_loss),
...
library(tidyr)
val_loss %>% 
  gather(model, loss, original_model, smaller_model) %>% 
...
val_loss %>% 
  gather(model, loss, original_model, bigger_model) %>% 
  ggplot(aes(x = epoch, y = loss, colour = model)) +
  geom_line()
```
...
train_loss <- data.frame(
  epoch = seq_along(original_hist$metrics$loss),
...
`regularizer_l2(0.001)` means every coefficient in the weight matrix of the layer will add `0.001 * weight_coefficient_value` to the total loss of 
...
val_loss %>% 
  gather(model, loss, original_model, l2_model) %>% 
  ggplot(aes(x = epoch, y = loss, colour = model)) +
  geom_line()
```
...
val_loss %>% 
  gather(model, loss, original_model, dpt_model) %>% 
  ggplot(aes(x = epoch, y = loss, colour = model)) +
  geom_line()
```
